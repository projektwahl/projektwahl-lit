...
export class MyCustomResolver implements Resolver {
  ...
  async getCandidates(
    ...
    const nextDescriptor = structUtils.parseDescriptor(
      `${descriptor.scope ? `@${descriptor.scope}/` : ""}${
        descriptor.name
      }@npm:${descriptor.range.slice(MyCustomResolver.PROTOCOL.length)}`,
      true
    );
    ...
    return candidates.map((locator) => {
      if (
        locator.scope === "open-wc" &&
        locator.name == "dev-server-hmr" &&
        locator.reference === "npm:0.1.2"
      ) {
        ...
        return structUtils.makeLocator(
          structUtils.convertToIdent(locator),
