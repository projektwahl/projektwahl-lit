...
async function gitClonePackage([pkgpath, pkg], package_json, refs) {
  ...
  let directory = join(
    "/tmp/npm-audit",
    Buffer.from(package_json.repository).toString("base64")
  );
  let result;
  if (!existsSync(directory)) {
    result = await asyncExecFile("git", [
      "clone",
      "--filter=tree:0",
      "--no-checkout",
  ...
  throw new Error(
    `no ref found from ${refs} for ${package_json.name} ${package_json.version} on ${package_json.repository}`
  );
}
...
async function typesPackage([pkgpath, pkg], package_json) {
  ...
  let commit = await asyncExecFile(
    "git",
    ["rev-list", "-1", `--before="${readme_time}"`, "master"],
    {
      cwd: directory,
    }
  );
...
for (const [pkgpath, pkg] of Object.entries(package_lock.packages)) {
  ...
  let package_json = JSON.parse(
    await readFile(join("original_" + pkgpath, "package.json"))
  );
  ...
  package_json.repository = package_json.repository.replace(
    "git@github.com:",
    "https://github.com/"
  );
  package_json.repository = package_json.repository.replace(
  ...
  if (
    package_json.repository === "https://github.com/dfcreative/color-name.git"
  ) {
  ...
  if (pkgpath.includes("/@types/")) {
    ...
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      `${pkg.version}-npm-packages`,
    ]);
  } else if (pkgpath.endsWith("/lit-html")) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      `lit-html@${pkg.version}`,
    ]);
  } else if (pkgpath.endsWith("/lit-element")) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      `lit-element@${pkg.version}`,
    ]);
    ...
    package_json.name === "uri-js" &&
    package_json.version === "4.4.1"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
    ...
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "a97f6adcc37ee1cad06ab7dc9b0bd842bbc5c664",
    ]);
  } else if (
    package_json.name === "string_decoder" &&
    package_json.version === "0.10.31"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "06bb4afbf163c9e1acd14125618784f9513f39d9",
    ]);
  } else if (
    package_json.name === "pstree.remy" &&
    package_json.version === "1.1.8"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "f82ab879fc8929a5be76828545a0674af14f8d43",
    ]);
  } else if (
    package_json.name === "progress" &&
    package_json.version === "2.0.3"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "0790207ef077cbfb7ebde24a1dd9895ebf4643e1",
    ]);
  } else if (
    package_json.name === "path-to-regexp" &&
    package_json.version === "0.1.7"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "4c5412af6fae141f48c32e535bc931573ade99c4",
    ]);
  } else if (
    package_json.name === "normalize-url" &&
    package_json.version === "4.5.1"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "454970b662086e8856d1af074c7a57df96545b8b",
    ]);
  } else if (
    package_json.name === "mkdirp" &&
    package_json.version === "0.5.5"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "f2003bbcffa80f8c9744579fabab1212fc84545a",
    ]);
  } else if (
    package_json.name === "json-buffer" &&
    package_json.version === "3.0.0"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "da6fe4c61fd9a5e7b450aecb079219794733b245",
    ]);
  } else if (
    package_json.name === "functional-red-black-tree" &&
    package_json.version === "1.0.1"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "e7c9899a68797f8e891220b4c1a70456991a32a5",
    ]);
  } else if (
    package_json.name === "fill-range" &&
    package_json.version === "7.0.1"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "39f421b499d5c97b62e955c179fa34c062aab2a5",
    ]);
  } else if (
    package_json.name === "@open-wc/dev-server-hmr" &&
    package_json.version === "0.1.2"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "@open-wc/dev-server-hmr@0.1.2-next.0",
    ]);
  } else if (
    package_json.name === "file-entry-cache" &&
    package_json.version === "6.0.1"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "c227beb3f0dacc1a8dd95504deabb74a7618c003",
    ]);
  } else if (
    package_json.name === "acorn-jsx" &&
    package_json.version === "5.3.2"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "f5c107b85872230d5016dbb97d71788575cda9c3",
    ]);
  } else if (
    package_json.name === "@swc/helpers" &&
    package_json.version === "0.2.13"
  ) {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      "a1a348372ff2c2d1941bbd5eb944de9f6da26628",
    ]);
  } else {
    correct_url = await gitClonePackage([pkgpath, pkg], package_json, [
      pkg.version,
