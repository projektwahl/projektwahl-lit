import { literal, sql, sqlToString } from './sql/index.js'
...
export function fetchData(table, fieldsToSelect, orderByInfo, query, sanitizedData, customFilterQuery) {
    ...
    let queries = query.sorting.map((value, index, array) => {
        ...
        return sql`(SELECT ${literal(fieldsToSelect.join(", "))} FROM ${literal(table)} WHERE ${query.paginationCursor === null} OR (${customFilterQuery(sanitizedData)}) AND (${part.map((value, index) => {
            sql`${index === part.length - 1 ? (value[1] === "ASC" ? sql`<` : sql`>`) : sql`IS NOT DISTINCT FROM`} ${value[0] ?? null} AND `
        })} TRUE) LIMIT ${query.paginationLimit+1})`
    });

    return sql`${queries.flatMap(v => [sql` UNION ALL `, v]).slice(1)} LIMIT ${query.paginationLimit+1}.`
}
