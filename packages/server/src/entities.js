 ...
 * @returns {Sql}
 */
export function fetchData(table, fieldsToSelect, orderByInfo, _query, sanitizedData, customFilterQuery) {
    const query = _query;
    ...
    if (query.paginationCursor === null) {
        ...
        let queries = query.sorting.map((value, index, array) => {
            ...
            return sql`(SELECT ${literal(fieldsToSelect.join(", "))} FROM ${literal(table)} WHERE${customFilterQuery(sanitizedData)} (${parts}) ORDER BY ${orderByQuery} LIMIT ${query.paginationLimit+1})`
        });    
