...
export function andThen<
  ...
  E extends { [key: string]: string } = { [key in keyof T]: string },
  U
>(result: Result<T, E>, op: (v: T) => Result<U, E>): Result<U, E> {
  if (!isOk(result)) {
    return result;
  }
...
export function mapOr<
  ...
  E extends { [key: string]: string } = { [key in keyof T]: string },
  Q
>(result: OptionalResult<T, E>, fun: (val: T) => Q, defaultResult: Q): Q {
  if (isOk(result)) {
...
const zoderr2err = (arr: z.ZodIssue[]) => {
  const initialValue = {};
  return arr.reduce((acc: Record<string, string>, cval) => {
...
export function zod2result<Input, Output>(
  input:
    | {
        success: true;
        data: Output;
