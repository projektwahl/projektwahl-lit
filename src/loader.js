...
const fileExists = (/** @type {import("fs").PathLike} */ path) => stat(path).then(() => true, () => false);
 ...
 */
export async function resolve(/** @type {string} */ specifier, context, defaultResolve) {
  ...
  if (targetUrl.href.startsWith("file://") && await fileExists(fileURLToPath(targetUrl.href))) {
    return {
      url: targetUrl.href
    };
  }
...
export async function load(url, context, defaultLoad) {
  if (/\.ts$/.test(url)) {
    //console.log("a")
    const { source: rawSource } = await defaultLoad(url, { format: "module" }, defaultLoad);
    //console.log(rawSource)
    const transformedSource = (await esbuild.transform(rawSource.toString(), {
        format: "esm",
        loader: "ts",
        sourcemap: "inline",
        sourcefile: url
