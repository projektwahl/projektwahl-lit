...
export const get = async () => {
  ...
  await sql.begin(async (sql) => {
    ...
    const projects =
      await sql // : Existing<RawProjectType>[]
    ...
    const users =
      await sql // : Existing<RawUserVoterType>[]
    ...
    for (const u of users) {
      await fileHandle.write(
        ` [user${u.id}] ${
          u.project_leader_id ? `project${u.project_leader_id}` : `null`
        }`
      );
    }
    await fileHandle.write(`;${os.EOL}`);
    ...
    await fileHandle.write(
      `param projects : min_participants max_participants :=${os.EOL}`
    );
    for (const p of projects) {
      await fileHandle.write(
        `project${p.id} ${p.min_participants} ${p.max_participants}${os.EOL}`
      );
    }
    await fileHandle.write(`;${os.EOL}`);
    ...
    // RawChoiceType[]
    const choices = await sql.file(
      "src/server/routes/evaluate/calculate.sql",
      [],
      {
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify(
        JSON5.parse(await readFile(outputFilePath3, "utf8"))
      ), // DONT ASK: trailing commas are cool
    };
  } catch (error) {
    console.log(error);
    return {
